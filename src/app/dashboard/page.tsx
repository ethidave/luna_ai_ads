...
export default function DashboardPage() {
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (data.success) {
        ...
        setPackagesError(data.message || "Failed to load packages");
        setPackages([]);
      }
    } catch (error) {
  ...
  const checkPackageAccess = async () => {
    try {
      ...
      const currentResponse = await fetch("/api/packages/current");
      if (currentResponse.ok) {
        const currentData = await currentResponse.json();
        if (currentData.hasActivePackage) {
      ...
      const upgradesResponse = await fetch("/api/packages/upgrades");
      ...
      if (packagesResponse.ok) {
        ...
        console.log(
          "📦 All packages count:",
          packagesData.allPackages?.length || 0
        );

        if (packagesData.allPackages && packagesData.allPackages.length > 0) {
          console.log(
            "📦 Using API packages:",
            packagesData.allPackages.map((p: any) => `${p.name} - $${p.price}`)
          );
          setAvailablePackages(packagesData.allPackages);
        } else {
  ...
  const fetchRealAdsData = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/ads/real-data");
      if (response.ok) {
        const data = await response.json();
  ...
  const handleUpgradePackage = (upgradePackage: any) => {
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white/70 text-sm sm:text-base">
                    {currentPackage.daysRemaining > 0
                      ? `${currentPackage.daysRemaining} days remaining`
                      : "Package expired - please renew"}
                  </p>
                </div>
              </div>
              {currentPackage.daysRemaining <= 7 && (
                <button
                  onClick={() => setActiveTab("packages")}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div className="bg-white rounded-lg p-4">
                      ...
                      <div className="text-lg font-semibold">
                        {currentPackage.maxFacebookAccounts}
                      </div>
                    </div>
                    <div className="bg-white rounded-lg p-4">
                      <div className="text-sm text-gray-500">
                      ...
                      <div className="text-lg font-semibold">
                        $
                        {currentPackage.dailyBudgetCap?.toLocaleString() ||
                          "Unlimited"}
                      </div>
                    </div>
                    <div className="bg-white rounded-lg p-4">
                      ...
                      <div className="text-lg font-semibold">
                        {currentPackage.hasTeamCollaboration ? "Yes" : "No"}
                      </div>
                    </div>
                  </div>
            ...
            >
              ...
              <CampaignsSection
                ...
                onGenerateTags={async (adId) => {
                  try {
                    const response = await fetch("/api/ads/generate-tags", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ adId }),
                    });
          ...
          {activeTab === "analytics" && (
            ...
            >
              <EnhancedAnalyticsSection
                analyticsData={realAdsData}
                performanceMetrics={summaryStats}
                trendData={realAdsData}
                campaigns={realAdsData}
                    ...
                    {packages.map((pkg, index) => (
                      <motion.div
                        ...
                        className={`relative group ${
                          pkg.isPopular ? "sm:scale-105 lg:scale-110" : ""
                        }`}
                      >
                        {/* Popular Badge */}
                        {pkg.isPopular && (
                    ...
                    {availableUpgrades.map((upgrade) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>Max Accounts: {upgrade.maxFacebookAccounts}</div>
                          <div>
                            Budget Cap: $
                            {upgrade.dailyBudgetCap?.toLocaleString() ||
                              "Unlimited"}
                          </div>
                          <div>
                            Team Collab:{" "}
                            {upgrade.hasTeamCollaboration ? "Yes" : "No"}
                          </div>
                          <div>
                            Dedicated Support:{" "}
                            {upgrade.hasDedicatedConsultant ? "Yes" : "No"}
                          </div>
                        </div>
                        <button
                          onClick={() => {
                ...
                <div className="p-6">
                  ...
                  <div className="mb-6">
                    <p className="text-gray-600 mb-4">
                      Are you sure you want to cancel your current package? This
