...
import { aiOptimizationService } from '@/lib/services/aiOptimization';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const optimization = await aiOptimizationService.optimizeAd(ad);

    // Update the ad with optimization suggestions
    ad.improvementSuggestions = optimization.suggestions;
    ...
    return NextResponse.json({
      success: true,
      optimization,
      message: 'Ad optimization completed successfully'
    });
