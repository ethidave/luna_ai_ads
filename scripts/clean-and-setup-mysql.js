...
async function cleanAndSetupMySQL() {
  let connection;

  try {
    console.log("🧹 Cleaning and setting up MySQL database...");
    ...
    await tempConnection.execute(
      `DROP DATABASE IF EXISTS \`${mysqlConfig.database}\``
    );
    console.log(`🗑️  Dropped existing database '${mysqlConfig.database}'`);

    // Create fresh database
    await tempConnection.execute(
      `CREATE DATABASE \`${mysqlConfig.database}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`
    );
    console.log(`✅ Created fresh database '${mysqlConfig.database}'`);
    ...
    console.log("🎉 MySQL database cleaned and set up successfully!");
    console.log("📝 You can now start your application with: npm run dev");
  } catch (error) {
    console.error("❌ Error cleaning and setting up MySQL database:", error);
    process.exit(1);
  } finally {
    if (connection) {
...
cleanAndSetupMySQL().catch(console.error);
